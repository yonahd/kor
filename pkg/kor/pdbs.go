...
func GetUnusedPdbs(includeExcludeLists IncludeExcludeLists, clientset kubernetes.Interface, slackParams ...string) {
	...
	if len(slackParams) == 1 {
		...
		if err := SendFileToSlack(outputFilePath, "Unused Pdbs", slackParams[0], slackParams[1]); err != nil {
			fmt.Fprintf(os.Stderr, "Failed to send output to Slack: %v\n", err)
		}
	} else {
